...
export const register = (app: express.Application) => {
    ...
    // simple sanity check
    app.get(`/api/sanity-check`, async (req: express.Request, res: express.Response) => {
        res.json({ message: "Ok" });
    });
    ...
    app.post(`/api/guitars/add`, oidc.ensureAuthenticated(), async (req: any, res: express.Response) => {
        try {
            const userId = req.userContext.userinfo.sub;
    ...
    app.post(`/api/guitars/update`, oidc.ensureAuthenticated(), async (req: any, res: express.Response) => {
        try {
            const userId = req.userContext.userinfo.sub;
            const id = await db.one(`
                UPDATE guitars
    ...
    app.delete(`/api/guitars/remove/:id`, oidc.ensureAuthenticated(), async (req: any, res: express.Response) => {
        try {
            const userId = req.userContext.userinfo.sub;
